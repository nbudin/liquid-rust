...
pub trait ObjectView: ValueView {
    ...
    /// Returns an iterator .
    fn iter<'k>(&'k self) -> Box<dyn Iterator<Item = (KStringCow<'k>, ValueCow<'k>)> + 'k>;
...
impl ObjectView for Object {
    ...
    fn values<'k>(&'k self) -> Box<dyn Iterator<Item = ValueCow<'k>> + 'k> {
        let i = Object::values(self).map(|v| ValueCow::Shared(Rc::new(v.as_view())));
        Box::new(i)
    }

    fn iter<'k>(&'k self) -> Box<dyn Iterator<Item = (KStringCow<'k>, ValueCow<'k>)> + 'k> {
        let i = Object::iter(self)
            .map(|(k, v)| (k.as_str().into(), ValueCow::Shared(Rc::new(v.as_view()))));
        Box::new(i)
    }
    ...
    fn get<'s>(&'s self, index: &str) -> Option<ValueCow<'s>> {
        Object::get(self, index).map(|v| ValueCow::Shared(Rc::new(v.as_view())))
    }
}
...
impl<K: ObjectIndex, V: ValueView, S: ::std::hash::BuildHasher> ObjectView for HashMap<K, V, S> {
    ...
    fn values<'k>(&'k self) -> Box<dyn Iterator<Item = ValueCow<'k>> + 'k> {
        let i = HashMap::values(self).map(|v| ValueCow::Shared(Rc::new(as_view(v))));
        Box::new(i)
    }

    fn iter<'k>(&'k self) -> Box<dyn Iterator<Item = (KStringCow<'k>, ValueCow<'k>)> + 'k> {
        let i = HashMap::iter(self)
            .map(|(k, v)| (k.as_index().into(), ValueCow::Shared(Rc::new(as_view(v)))));
        Box::new(i)
    }
    ...
    fn get<'s>(&'s self, index: &str) -> Option<ValueCow<'s>> {
        HashMap::get(self, index).map(|v| ValueCow::Shared(Rc::new(as_view(v))))
    }
}
...
impl<K: ObjectIndex, V: ValueView> ObjectView for BTreeMap<K, V> {
    ...
    fn values<'k>(&'k self) -> Box<dyn Iterator<Item = ValueCow<'k>> + 'k> {
        let i = BTreeMap::values(self).map(|v| ValueCow::Shared(Rc::new(as_view(v))));
        Box::new(i)
    }

    fn iter<'k>(&'k self) -> Box<dyn Iterator<Item = (KStringCow<'k>, ValueCow<'k>)> + 'k> {
        let i = BTreeMap::iter(self)
            .map(|(k, v)| (k.as_index().into(), ValueCow::Shared(Rc::new(as_view(v)))));
        Box::new(i)
    }
    ...
    fn get<'s>(&'s self, index: &str) -> Option<ValueCow<'s>> {
        BTreeMap::get(self, index).map(|v| ValueCow::Shared(Rc::new(as_view(v))))
    }
}
